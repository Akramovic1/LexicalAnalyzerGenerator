...
*/
    Parser p;
    p.parseFile(R"(C:\Users\ahmed\Downloads\LexicalAnalyzerGenerator\grammar.txt)");
    NFA_Generator g;
    g.generate_all_NFAs(Parser::RE_expressions,p.raw_RE_definitions);
    Identifier i;
    i.parse_string(R"(C:\Users\ahmed\Downloads\LexicalAnalyzerGenerator\input.txt)");
...
//    cout << Parser::tokens["if"].first << Parser::tokens["id"].first;
    ...
    fstream newfile;
    string sourceinput ;
    newfile.open(R"(C:\Users\ahmed\Downloads\LexicalAnalyzerGenerator\input.txt)",ios::in); //open a file to perform read operation using file object
